...
1. Command Blocker Hook - denies dangerous bash commands
2. Audit Logger Hook - logs all tool invocations
"""
import asyncio
import sys
import os
...
async def test_command_blocker_denies_dangerous_commands():
    """Test that command blocker denies dangerous bash commands"""
    print("\n--- Testing Command Blocker: Deny Dangerous Commands ---")
    ...
    for test_input in dangerous_commands:
        result = await block_dangerous_commands(test_input, "test-id", {})
...
async def test_audit_logger_logs_tool_invocations():
    """Test that audit logger logs all tool invocations"""
    print("\n--- Testing Audit Logger: Log Tool Invocations ---")
    ...
    for test_input in test_cases:
        result = await log_tool_use(test_input, "test-tool-id", {})
...
async def test_hook_return_format():
    ...
    # Test deny format
    deny_result = await block_dangerous_commands(
        {"tool_name": "bash", "tool_input": {"command": "rm -rf /"}},
        "test-id",
        {}
    )
    ...
    # Test no-op format
    no_op_result = await block_dangerous_commands(
...
async def run_tests():
    ...
    tests = [
        ("Command Blocker - Deny Dangerous", test_command_blocker_denies_dangerous_commands),
        ("Command Blocker - Allow Safe", test_command_blocker_allows_safe_commands),
        ("Audit Logger - Log Invocations", test_audit_logger_logs_tool_invocations),
